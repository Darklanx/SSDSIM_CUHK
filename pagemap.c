...
struct ssd_info *pre_process_page(struct ssd_info *ssd)
{
	...
	while(fgets(buffer_request,200,ssd->tracefile))
	{
        ...
        sscanf(buffer_request, "%d %lld %s %d %s %lld %d %d", &io_num, &time, &dump, &dump2, &type, &offset, &size, &dump3);
		...
		#endif 
		// end NARGES 
        lsn = (unsigned int)(offset / (ssd->parameter->subpage_capacity) % largest_lsn);
		time = (time / (unsigned long long)ssd->time_scale);
		fl++;
...
struct ssd_info *get_ppn(struct ssd_info *ssd,unsigned int channel,unsigned int chip,unsigned int die,unsigned int plane,struct sub_request *sub)
{
	...
	unsigned int active_block;
	unsigned int block;
	unsigned int page,flag=0,flag1=0, flag_update = 0;
    ...
    if(ssd->parameter->page_level_erase == 1){
        // 如果使用page erase需要紀錄可以用的page 但這邊為了方便我們讓她用while來隨便可以用的 之後可以優化

        while(page < ssd->parameter->page_block){
            if(ssd->channel_head[channel].chip_head[chip].die_head[die].plane_head[plane].blk_head[active_block].page_head[page].free_state == PG_SUB){
                ssd->channel_head[channel].chip_head[chip].die_head[die].plane_head[plane].blk_head[active_block].last_write_page = page;	
                break;
            }
            page++;
        }
        if (page == ssd->parameter->page_block){
            printf("Valid page not found\n");
            exit(-1);
        }
    }
    ...
    /*这个逻辑页进行了更新，需要将原来的页置为失效*/
	{
		...
		{
            printf("\nchannel: %d\n",location->channel);
			printf("2. Error in get_ppn()\n");
            printf("lpn: %d\n",lpn);

            printf("location lpn: %d\n",ssd->channel_head[location->channel].chip_head[location->chip].die_head[location->die].plane_head[location->plane].blk_head[location->block].page_head[location->page].lpn);
            exit(-1);
		}

        ///*表示某一页失效，同时标记valid和free状态都为0*/
		ssd->channel_head[location->channel].chip_head[location->chip].die_head[location->die].plane_head[location->plane].blk_head[location->block].page_head[location->page].valid_state=0; // valid state = 0
		ssd->channel_head[location->channel].chip_head[location->chip].die_head[location->die].plane_head[location->plane].blk_head[location->block].page_head[location->page].free_state=0;  // free state = 0

		ssd->channel_head[location->channel].chip_head[location->chip].die_head[location->die].plane_head[location->plane].blk_head[location->block].page_head[location->page].lpn=0;
		ssd->channel_head[location->channel].chip_head[location->chip].die_head[location->die].plane_head[location->plane].blk_head[location->block].invalid_page_num++;
        
        if (ssd->parameter->page_level_erase == 0){
            // here: Erase?
            if (ssd->channel_head[location->channel].chip_head[location->chip].die_head[location->die].plane_head[location->plane].blk_head[location->block].invalid_page_num == ssd->parameter->page_block)
            {
                ...
                new_direct_erase->type = 0; // block erase
        ...
        } else if (ssd->parameter->page_level_erase == 1){
            new_direct_erase=(struct direct_erase *)malloc(sizeof(struct direct_erase));
            alloc_assert(new_direct_erase,"new_direct_erase");
            memset(new_direct_erase,0, sizeof(struct direct_erase));
            new_direct_erase->block = location->block;
            new_direct_erase->type = 1; // page erase
            new_direct_erase->next_node = NULL;
            new_direct_erase->page = location->page;
            direct_erase_node=ssd->channel_head[location->channel].chip_head[location->chip].die_head[location->die].plane_head[location->plane].erase_node;
            ...
            gc_node->block=0xffffffff; // useless now
            gc_node->page=0; // useless now
            gc_node->type = 1;
            gc_node->state=GC_WAIT;
            gc_node->priority=GC_UNINTERRUPT;
            gc_node->next_node=ssd->channel_head[location->channel].gc_command;
            ssd->channel_head[location->channel].gc_command=gc_node;
            ssd->gc_request++;
	...
	{                                                  
        ...
        //if(ssd->parameter->page_block * ssd->parameter->block_plane != ssd->channel_head[channel].chip_head[chip].die_head[die].plane_head[plane].free_page)
		{
            //printf("new gc request\n");
			gc_node=(struct gc_operation *)malloc(sizeof(struct gc_operation));
			alloc_assert(gc_node,"gc_node");
			memset(gc_node,0, sizeof(struct gc_operation));
...
{
	//printf("Erasing in block level (%d, %d, %d, %d, %d)\n", channel, chip, die, plane, block); 
	unsigned int i=0;
...
Status erase_page(struct ssd_info * ssd,unsigned int channel ,unsigned int chip ,unsigned int die ,unsigned int plane ,unsigned int block, unsigned int page)
{      
	//printf("Erasing (%d, %d, %d, %d, %d, %d)\n", channel, chip, die, plane, block, page); 
	unsigned int i=0;
    ...
    ssd->channel_head[channel].chip_head[chip].die_head[die].plane_head[plane].blk_head[block].page_head[page].free_state=PG_SUB;
    ssd->channel_head[channel].chip_head[chip].die_head[die].plane_head[plane].blk_head[block].page_head[page].valid_state=0;
    ssd->channel_head[channel].chip_head[chip].die_head[die].plane_head[plane].blk_head[block].page_head[page].lpn=-1;

	ssd->erase_count++;
	ssd->channel_head[channel].erase_count++;			
	ssd->channel_head[channel].chip_head[chip].erase_count++;
	ssd->channel_head[channel].chip_head[chip].die_head[die].plane_head[plane].free_page += 1;
...
Status erase_planes(struct ssd_info * ssd, unsigned int channel, unsigned int chip, unsigned int die1, unsigned int plane1,unsigned int command)
{
    ...
    unsigned int page = 0; // if erase in page
	...
	{
        printf("weird command in erase planes\n");
        exit(-1);
		return ERROR;
	}
    ...
    // 在下面NORMAL的時候 如果發現是page erase 會設定channel state 為 idle
	//ssd->channel_head[channel].current_state=CHANNEL_IDLE;
    ...
    //printf("state: %d\n",ssd->channel_head[channel].chip_head[chip].current_state);
    //ssd->channel_head[channel].chip_head[chip].current_state=CHIP_IDLE;
	...
	else if(command==NORMAL)                                             /*普通命令NORMAL的处理*/
	{
		...
		int type = direct_erase_node->type;
        if (type != 1){ // 為什麼要這樣寫呢 因為type是我家的 我怕type沒有給值 之後應該要把所有有創立這個object都去設定type
		    erase_operation(ssd,channel,chip,die1,plane1,block);
        }
        else{
            page = direct_erase_node->page;    
            // page erase不應該佔用channel? 有待商榷
            //ssd->channel_head[channel].current_state=CHANNEL_IDLE;
            erase_page(ssd,channel,chip,die1,plane1,block,page);
        }
		...
		ssd->channel_head[channel].chip_head[chip].next_state_predict_time=ssd->channel_head[channel].next_state_predict_time+ssd->parameter->time_characteristics.tWB+1*ssd->parameter->time_characteristics.tBERS;	
	...
	if(((direct_erase_node)!=NULL)&&(direct_erase_node->block==block1) && ssd->parameter->page_level_erase !=1)
	{
...
int uninterrupt_gc(struct ssd_info *ssd,unsigned int channel,unsigned int chip,unsigned int die,unsigned int plane)       
{
	...
	{		
		...
		{   
            ...
            printf("block : %u\n", block);
            printf("block free:%d\n",ssd->channel_head[channel].chip_head[chip].die_head[die].plane_head[plane].blk_head[block].free_page_num);
            printf("free state: %d\n",ssd->channel_head[channel].chip_head[chip].die_head[die].plane_head[plane].blk_head[block].page_head[i].free_state);
			printf("\ntoo much free page. \t %d\t .%d\t%d\t%d\t%d\t%d\t%d\t\n",free_page,channel,chip,die,plane,block,i);
...
Status gc_for_channel(struct ssd_info *ssd, unsigned int channel)
{
	...
	while (gc_node!=NULL) // 第一次遍歷只是要找flag_priority
	{
	...
	if (flag_priority!=1) // 這邊來找第一個可以處理gc的傢伙 break時gc_node 指向的傢伙就是惹                                
	{
		gc_node=ssd->channel_head[channel].gc_command;
		while (gc_node!=NULL)
		{
	...
	if (gc_node->priority==GC_UNINTERRUPT) // 這個跟get_ppn最後面有關(也就是跟參數active_write有關)
	{
		...
		if (flag_direct_erase!=SUCCESS)
		{
            ...
            //if( ssd->parameter->page_level_erase)
            //exit(-1);
