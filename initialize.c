...
*******************************************/
struct ssd_info *initiation(struct ssd_info *ssd, char *trace_file, char *add_name, char *time_scale) {
    ...
    sprintf(ssd->parameterfilename, "page.parameters_%s", add_name);

    strncpy(ssd->tracefilename, trace_file, 25);
    //printf("\ninput trace file name:");
    //scanf("%s",ssd->tracefilename);
    //printf("%s\n", time_scale);
    int _time_scale = strtod(time_scale, NULL);
    parameters = load_parameters(ssd->parameterfilename);
    //printf("%lf\n", _time_scale);
    if (_time_scale <= 0) {
        printf("Invalid time_sacle\n");
        exit(-1);
    }
    ssd->time_scale = _time_scale;
    sprintf(name, "output_%s_%s.%s.%d", trace_file, add_name, time_scale, parameters->page_level_erase);
    strncpy(ssd->outputfilename, name, 30);

    sprintf(name, "stat1_%s_%s.%s", trace_file, add_name, time_scale);
    strncpy(ssd->statisticfilename, name, 30);
    ...
    if (ssd->statisticfile == NULL) {
        printf("the statistic file 2 can't open\n");
        return NULL;
    }
...
struct parameter_value *load_parameters(char parameter_file[30]) {
    ...
    p->queue_length = 32; // NARGES why 5, it's better to be more than 5, like 32 or 20  // FIXME define it or make it a parameter
    ...
    while (fgets(buf, 200, fp)) {
        ...
        } else if ((res_eql = strcmp(buf, "initial valid")) == 0) {
            sscanf(buf + next_eql, "%d", &p->initial_valid);
        } else if ((res_eql = strcmp(buf, "valid ratio")) == 0) {
            sscanf(buf + next_eql, "%f", &p->valid_ratio);
